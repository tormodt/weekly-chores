...
export class SimpleFirestoreService {
    constructor() {
    ...
    async addTask(task) {
        ...
        try {
            ...
            const cleanTaskData = Object.fromEntries(Object.entries(taskData).map(([key, value]) => [key, value === undefined ? null : value]));
            const docRef = await firebaseUtils.addDoc(firebaseUtils.collection(this.db, `years/${year}/weeks/${week}/tasks`), cleanTaskData);
    ...
    async subscribeToTasks(callback) {
        ...
        try {
            ...
            const tasksRef = firebaseUtils.collection(this.db, `years/${year}/weeks/${week}/tasks`);
            firebaseUtils.onSnapshot(tasksRef, (snapshot) => {
    ...
    async subscribeToTasksForWeek(year, week, callback) {
        await this.initPromise;
        if (!this.initialized) {
            throw new Error('Firebase not initialized');
        }
    ...
    async addPendingApproval(approval) {
        ...
        try {
            const docRef = await firebaseUtils.addDoc(firebaseUtils.collection(this.db, 'pendingApprovals'), { ...approval, createdAt: new Date() });
    ...
    async subscribeToPendingApprovals(callback) {
        ...
        try {
            ...
            firebaseUtils.onSnapshot(approvalsRef, (snapshot) => {
    ...
    async testConnection() {
        ...
        try {
            ...
            const docRef = await firebaseUtils.addDoc(firebaseUtils.collection(this.db, 'test'), testDoc);
            console.log('✅ Test document created successfully');
            // Clean up
            await firebaseUtils.deleteDoc(firebaseUtils.doc(this.db, 'test', docRef.id));
    ...
    async addRecurringTask(task) {
        ...
        try {
            ...
            const docRef = await firebaseUtils.addDoc(firebaseUtils.collection(this.db, 'recurringTasks'), taskData);
    ...
    async subscribeToRecurringTasks(callback) {
        await this.initPromise;
        if (!this.initialized) {
            throw new Error('Firebase not initialized');
        }
